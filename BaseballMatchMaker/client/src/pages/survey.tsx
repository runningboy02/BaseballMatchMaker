...
export default function Survey() {
  ...
  const submitSurveyMutation = useMutation({
    mutationFn: async (answers: Array<{ questionId: number; selectedOption: string }>) => {
      const response = await apiRequest("POST", "/api/survey/submit", { answers });
      return response.json();
    },
    onSuccess: (data) => {
      setLocation(`/result/${data.shareId}`);
    },
    onError: () => {
      toast({
        title: "오류",
  ...
  const isNextDisabled = !selectedOption || submitSurveyMutation.isPending;
